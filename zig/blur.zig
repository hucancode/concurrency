...
fn blurHorizontalSIMD(
    ...
    // Only use SIMD for 4-channel images (RGBA)
    if (channels == 4) {
            ...
            for (0..width) |x| {
                ...
                while (k + SIMD_WIDTH <= kernel_size) : (k += SIMD_WIDTH) {
                    ...
                    if (base_x_signed >= 0 and base_x_signed + SIMD_WIDTH <= @as(i32, @intCast(width))) {
                        ...
                        const g_data = @as(@Vector(SIMD_BYTES, u8), @bitCast(@as(@Vector(SIMD_BYTES/4, u32), @bitCast(pixel_data)) >> @splat(8)));
                        const g_masked = @select(u8, mask, g_data, zero);
                        
                        // Extract B channel (bytes 2, 6, 10, 14, ...) by shifting pixel_data
                        const b_data = @as(@Vector(SIMD_BYTES, u8), @bitCast(@as(@Vector(SIMD_BYTES/4, u32), @bitCast(pixel_data)) >> @splat(16)));
                        const b_masked = @select(u8, mask, b_data, zero);
                        
                        // Extract A channel (bytes 3, 7, 11, 15, ...) by shifting pixel_data
                        const a_data = @as(@Vector(SIMD_BYTES, u8), @bitCast(@as(@Vector(SIMD_BYTES/4, u32), @bitCast(pixel_data)) >> @splat(24)));
                        ...
                        // Multiply by weights and accumulate using @reduce
        ...
        // Fall back to scalar for non-4-channel images
        blurHorizontalScalar(src, dst, width, height, channels, kernel, radius, start_row, end_row);
    }
}

// Keep the original scalar version as fallback
fn blurHorizontalScalar(
    src: []const u8,
    dst: []u8,
    width: usize,
    height: usize,
