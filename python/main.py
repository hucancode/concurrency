...
from monte_carlo import monte_carlo_operation
import numpy as np
...
def main():
    if len(sys.argv) != 6:
        ...
        print("Filter types: blur, kuwahara, monte_carlo")
        print("For monte_carlo: radius represents number of samples")
        sys.exit(1)
    ...
    if filter_type == 'monte_carlo':
        samples = radius
        print(f"Monte Carlo Pi estimation with {samples} samples using {num_workers} workers")
    ...
    if filter_type not in ['blur', 'kuwahara']:
        ...
        print("Available filters: blur, kuwahara, monte_carlo")
        sys.exit(1)
